...
#RUN yum -y swap -- remove fakesystemd
#RUN yum -y install systemd systemd-libs
#RUN yum -y swap -- install systemd systemd-libs
RUN yum clean all; \
...
RUN /usr/local/src/install.sh --release stable --disable-fail2ban --ignore-hostname --silent ISPmanager-Lite
